<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Tambah Transaksi') }}</h2>
                            ...
                            <div>
                                ...
                                <select id="termin_id" name="termin_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach(\App\Models\Termin::all() as $termin)
                                        <option value="{{ $termin->id }}" {{ old('termin_id') == $termin->id ? 'selected' : '' }}>{{ $termin->name }}</option>
                                    @endforeach
                                </select>
